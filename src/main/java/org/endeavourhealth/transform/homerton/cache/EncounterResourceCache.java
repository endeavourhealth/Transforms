...
import static org.endeavourhealth.transform.barts.transformsOld.BasisTransformer.*;
...
public class EncounterResourceCache {
    ...
    public static EncounterBuilder getEncounterBuilder(HomertonCsvHelper csvHelper, String encounterId) throws Exception {

        ResourceId encounterResourceId = getEncounterResourceId(HomertonCsvToFhirTransformer.HOMERTON_RESOURCE_ID_SCOPE, encounterId);
    ...
    public static EpisodeOfCareBuilder getEpisodeBuilder(HomertonCsvHelper csvHelper, String episodeId) throws Exception {

        ResourceId episodeResourceId = getEpisodeOfCareResourceId(HomertonCsvToFhirTransformer.HOMERTON_RESOURCE_ID_SCOPE, episodeId);
    ...
    public static EpisodeOfCareBuilder getEpisodeBuilder(HomertonCsvHelper csvHelper, UUID resourceId) throws Exception {
    ...
    public static EpisodeOfCareBuilder createEpisodeBuilder(CsvCell episodeIdCell) throws Exception {
        ...
        ResourceId episodeResourceId = getEpisodeOfCareResourceId(HomertonCsvToFhirTransformer.HOMERTON_RESOURCE_ID_SCOPE, episodeIdCell.getString());

        if (episodeResourceId == null) {
            episodeResourceId = createEpisodeOfCareResourceId(HomertonCsvToFhirTransformer.HOMERTON_RESOURCE_ID_SCOPE, episodeIdCell.getString());
        }
    ...
    public static EncounterBuilder createEncounterBuilder(CsvCell encounterIdCell, CsvCell finIdCell) throws Exception {
        ...
        ResourceId encounterResourceId = getEncounterResourceId(HomertonCsvToFhirTransformer.HOMERTON_RESOURCE_ID_SCOPE, encounterIdCell.getString());

        if (encounterResourceId == null) {
            encounterResourceId = createEncounterResourceId(HomertonCsvToFhirTransformer.HOMERTON_RESOURCE_ID_SCOPE, encounterIdCell.getString());
        }
    ...
    public static void fileEncounterResources(FhirResourceFiler fhirResourceFiler) throws Exception {
        ...
        for (UUID encounterId: encounterBuildersByUuid.keySet()) {
            ...
            if (encounter.hasStatus() == false) {
                LOG.error("Data error. Saving EncounterTable without status.");
                error = true;
            }
            if (encounter.hasPeriod() == false || encounter.getPeriod().getStart() == null ) {
                LOG.error("Data error. Saving EncounterTable without dates.");
                error = true;
            }
            if (encounter.getIdentifier() == null || encounter.getIdentifier().size() == 0) {
                LOG.error("Data error. Saving EncounterTable without Identifiers.");
                error = true;
            }
            if (encounter.hasEpisodeOfCare() == false || encounter.getEpisodeOfCare().size() == 0) {
                LOG.error("Data error. Saving EncounterTable without EoC reference.");
                error = true;
            }
            if (encounter.hasLocation() == false || encounter.getLocation().size() == 0) {
                LOG.error("Data error. Saving EncounterTable without location reference.");
                error = true;
            }
            if (error) {
        ...
        for (UUID encounterId: deletedEncounterBuildersByUuid.keySet()) {
            LOG.trace("Deleting EncounterTable " + encounterId.toString());
