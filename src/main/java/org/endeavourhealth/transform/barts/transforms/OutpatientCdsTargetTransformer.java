...
public class OutpatientCdsTargetTransformer {
    ...
    private static void createOutpatientCdsEncounters(FhirResourceFiler fhirResourceFiler, BartsCsvHelper csvHelper) throws Exception {
        ...
        for (StagingOutpatientCdsTarget targetOutpatientCds : targetOutpatientCdsRecords) {
            ...
            if (encounterId != null) {
                ...
                deleteHL7ReceiverPatientOutpatientEncounters(parentEncounterBuilder, targetOutpatientCds, fhirResourceFiler, csvHelper);
                patientOutpatientEncounterDates.clear();
                patientOutpatientEpisodesDeleted.clear();
     ...
     */
    private static void deleteHL7ReceiverPatientOutpatientEncounters(EncounterBuilder existingDWParentEncounterBuilder,
                                                                     StagingOutpatientCdsTarget targetOutpatientCds,
                                                                     FhirResourceFiler fhirResourceFiler,
                                                                     BartsCsvHelper csvHelper) throws Exception {
        ...
        for (UUID patientId: patientIds) {
            ...
            for (ResourceWrapper wrapper : resourceWrappers) {
                ...
                if (existingHL7Encounter.hasPeriod()
                    ...
                    if (existingHL7Encounter.getClass_().equals(Encounter.EncounterClass.OUTPATIENT)) {
                        if (patientOutpatientEncounterDates.contains(existingHL7Encounter.getPeriod().getStart().getTime())) {
                            ...
                            LOG.debug("Existing Outpatient ADT encounterId: "+existingHL7Encounter.getId()+" deleted as matched type and date to DW");
