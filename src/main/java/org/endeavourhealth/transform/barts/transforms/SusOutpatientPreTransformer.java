...
public class SusOutpatientPreTransformer {
    ...
    private static void parsePrimaryProcedure(SusOutpatient parser, StagingCds commonContent, BartsCsvHelper csvHelper) throws Exception {
        ...
        if (dateCell.isEmpty()) {
            TransformWarnings.log(LOG, csvHelper, "Missing primary procedure date for outpatient CDS record", dateCell);
            return;
        }
    ...
    private static void parseSecondaryProcedure(SusOutpatient parser, StagingCds commonContent, BartsCsvHelper csvHelper) throws Exception {
        ...
        if (dateCell.isEmpty()) {
            TransformWarnings.log(LOG, csvHelper, "Missing secondary procedure date for outpatient CDS record", dateCell);
        ...
        for (String word : BartsSusHelper.splitEqually(otherProcedureOPCS.getString(), 40)) {
            ...
            if (word.length() > 4) {
                ...
                if (!Strings.isNullOrEmpty(dateStr)) {
                    ...
                    TransformWarnings.log(LOG, csvHelper, "Missing " + seq + " procedure date for outpatient CDS record", otherProcedureOPCS);
                    CsvCell dateCell = parser.getPrimaryProcedureDate();
                    if (dateCell.isEmpty()) {
                        TransformWarnings.log(LOG, csvHelper, "Skipping secondary procedure because primary date is empty", dateCell);
                        continue;
