...
public class SusOutpatientTransformer {
    ...
    private static Map<String, SusTailCacheEntry> processTailsFile(ParserI parser, Map<String, List<ParserI>> parserMap) throws Exception {
        SusOutpatientTail tailParser = (SusOutpatientTail)BartsCsvToFhirTransformer.findTailFile(parserMap, "SusOutpatientTail", parser.getFilePath());
    ...
    public static void mapFileEntry(SusOutpatient parser,
        ...
        if ((parser.getICDPrimaryDiagnosis().length() > 0) || (parser.getOPCSPrimaryProcedureCode().length() > 0)) {
            ...
            if (tr != null && tr.getEpisodeId() != null && tr.getEpisodeId().length() > 0 && tr.getEncounterId() != null && tr.getEncounterId().length() > 0) {
                ...
                //When partial update of EpisodeOfCare is implemented then the 'end date' should probably only be set once when/if creating the resource. It should not be updated here as HL7 is likely more accurate. Same might apply to 'start date'
