...
public class SusOutpatientTransformer {
    ...
    public static void transformProcedures(List<ParserI> parsers,
        ...
        for (ParserI parser: parsers) {
            ...
            while (parser.nextRecord()) {
                try {
                    processRecordProcedures((SusOutpatient)parser);
    ...
    private static Map<String, SusTailCacheEntry> processTailsFile(ParserI parser, Map<String, List<String>> fileMap) throws Exception {
        String tailFilePath = BartsCsvToFhirTransformer.findTailFile(fileMap, "SusOutpatientTail", parser.getFilePath());
        SusOutpatientTail tailParser = new SusOutpatientTail(parser.getServiceId(), parser.getSystemId(), parser.getExchangeId(), parser.getVersion(), tailFilePath);
        Map<String, SusTailCacheEntry> tailsCache = new HashMap<>();
        SusOutpatientTailPreTransformer.transform(tailParser, tailsCache);
        return tailsCache;
    }

    private static void processRecordProcedures(SusOutpatient parser) throws Exception {


        //TODO - copy from inpatient transform
    }

    private static void processProcedure(SusOutpatient parser, CsvCell code, CsvCell date, boolean isPrimary) throws Exception {

//TODO
    }

    private static void validateRecordType(SusOutpatient parser) throws Exception {
    ...
    ///////////BELOW IS THE SUS OUTPATIENT TRANSFORM FROM WHEN IT WAS ORIGINALLY WRITTEN FOR 2.1//////////
